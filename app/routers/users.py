...
async def create_user(user_register: UserRegister):
    # Comprobaciones básicas: usuario o email ya existe
    ...
    hashed = get_hash_password(user_register.password)  # ya devuelve str
    new_user = UserDb(
        id=None,
    ...
    new_id = insert_user(new_user)
    new_user.id = new_id

    # Devolvemos UserOut (no incluimos password en la respuesta)
